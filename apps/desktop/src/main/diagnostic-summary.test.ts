import type { ActionTimelineEntry, ReportableError } from '@dsr-codesign/shared';
...
describe('composeSummaryMarkdown', () => {
  it('renders header, environment, and "(no stack)" when stack is undefined', () => {
    ...
    expect(md).toContain('Generated by dsr-codesign 0.4.0.');
  });
  ...
  describe('path and URL redaction edge cases', () => {
    ...
    it('redacts /tmp paths', () => {
      const md = composeSummaryMarkdown(
        baseInput({
          error: baseEvent({ message: 'wrote /tmp/dsr-codesign-cache/foo then failed' }),
          includePaths: false,
        }),
      );
      expect(md).not.toContain('/tmp/dsr-codesign-cache');
      expect(md).toContain('[path omitted]');
    });
