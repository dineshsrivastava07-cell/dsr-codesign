...
describe('classifyMarkdownHref', () => {
  ...
  it('routes https URLs through the safe external-open path', () => {
    expect(classifyMarkdownHref('https://github.com/DSR-AI-Lab/dsr-codesign/issues/339')).toEqual({
      kind: 'external',
      url: 'https://github.com/DSR-AI-Lab/dsr-codesign/issues/339',
    });
  });

  it('normalizes surrounding whitespace on external URLs', () => {
    expect(classifyMarkdownHref('  https://github.com/DSR-AI-Lab/dsr-codesign/releases  ')).toEqual(
      {
        kind: 'external',
        url: 'https://github.com/DSR-AI-Lab/dsr-codesign/releases',
      },
    );
  });
