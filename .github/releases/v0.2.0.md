...
DSR CoDesign 0.2.0 changes the product shape. The app is no longer a one-shot prompt-to-artifact generator; each design now has a real workspace, a session history, and an agent loop that can read files, edit files, ask questions, preview its work, and stop only after a `done` check.
...
- Permissioned local tools. The agent can use read, write, edit, bash, grep, find, and ls through the DSR CoDesign permission UI.
...
Compare everything in this release: https://github.com/DSR-AI-Lab/dsr-codesign/compare/v0.1.4...v0.2.0
